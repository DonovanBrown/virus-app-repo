...
public class TakePhotoService extends Service {
     ...
     */
    private void save(Bitmap bitmapImage, boolean result) throws IOException {
        String flag = "";
        if (result) {
            flag = "ALARM";
        }
        String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new Date());
        file = new File(getExternalFilesDir(Environment.DIRECTORY_PICTURES).getAbsolutePath()  +  "/" + flag + greenValue(bitmapImage) + "greenValue" + timeStamp + ".jpg");
        try (OutputStream output = new FileOutputStream(file)) {
            bitmapImage.compress(Bitmap.CompressFormat.JPEG, 100, output);
            curStorage += file.length();
            output.flush();
        }
        if (detectArea != null) {
            File detectCrop = new File(getExternalFilesDir(Environment.DIRECTORY_PICTURES).getAbsolutePath()  + "/" + flag + greenValue(bitmapImage) + "greenValue" +  timeStamp + "detectCrop.jpg");
